...
class KeyPairStorageService {
    ...
    public constructor(private readonly walletConfig?: WalletConfig) {
        if (!walletConfig) {
            throw new WalletError('walletConfig is required')
        }
    ...
    private decryptData(encrypted: string, iv: string) {
        ...
        cipher.start(iv);
        cipher.update(util.createBuffer(Buffer.from(encrypted, "hex") as any));
        cipher.finish();
    ...
    private encryptData(data: Uint8Array) {
        ...
        cipher.start(iv);
        cipher.update(util.createBuffer(data as any));
        cipher.finish();
...
@injectable()
export class IDBBrowserWallet implements Wallet {
