import type { BaseRecord, BaseRecordConstructor, Query, StorageService, TagsBase } from '@credo-ts/core'
import { AgentContext, CredoError, type Wallet } from '@credo-ts/core'
...
export function assertBrowserWallet(wallet: Wallet): asserts wallet is IDBBrowserWallet {
    if (!(wallet instanceof IDBBrowserWallet)) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const walletClassName = (wallet as any).constructor?.name ?? 'unknown'
        throw new CredoError(`Expected wallet to be instance of IDBBrowserWallet, found ${walletClassName}`)
    }
}
