...
export class BrowserAnoncreds implements Anoncreds {
    createW3cCredential(options: { credentialDefinition: ObjectHandle; credentialDefinitionPrivate: ObjectHandle; credentialOffer: ObjectHandle; credentialRequest: ObjectHandle; attributeRawValues: Record<string, string>; revocationConfiguration?: NativeCredentialRevocationConfig; w3cVersion?: string; }): ObjectHandle {
        throw new Error('Method not implemented.');
    }
    processW3cCredential(options: { credential: ObjectHandle; credentialRequestMetadata: ObjectHandle; linkSecret: string; credentialDefinition: ObjectHandle; revocationRegistryDefinition?: ObjectHandle; }): ObjectHandle {
        throw new Error('Method not implemented.');
    }
    createW3cPresentation(options: { presentationRequest: ObjectHandle; credentials: NativeCredentialEntry[]; credentialsProve: NativeCredentialProve[]; linkSecret: string; schemas: Record<string, ObjectHandle>; credentialDefinitions: Record<string, ObjectHandle>; w3cVersion?: string; }): ObjectHandle {
        throw new Error('Method not implemented.');
    }
    verifyW3cPresentation(options: { presentation: ObjectHandle; presentationRequest: ObjectHandle; schemas: ObjectHandle[]; schemaIds: string[]; credentialDefinitions: ObjectHandle[]; credentialDefinitionIds: string[]; revocationRegistryDefinitions?: ObjectHandle[]; revocationRegistryDefinitionIds?: string[]; revocationStatusLists?: ObjectHandle[]; nonRevokedIntervalOverrides?: NativeNonRevokedIntervalOverride[]; }): boolean {
        throw new Error('Method not implemented.');
    }
